...
class AccountFollowupStatByPartner(models.Model):
    _name = "devit_account_followup.stat.by.partner"
    ...
    max_followup_id = fields.Many2one('devit_account_followup.followup.line',
    ...
    @api.model_cr
    def init(self):
        tools.drop_view_if_exists(self._cr, 'devit_account_followup_stat_by_partner')
        ...
        #  enough for a time.
        self._cr.execute("""
            create view devit_account_followup_stat_by_partner as (
                SELECT
...
class AccountFollowupSendingResults(models.TransientModel):
    ...
    _name = 'devit_account_followup.sending.results'
    _description = \
...
class AccountFollowupPrint(models.TransientModel):
    _name = 'devit_account_followup.print'
    _description = 'Print Follow-up & Send Mail to Customers'

    def _get_followup(self):
        if self.env.context.get('active_model',
                                'ir.ui.menu') == 'devit_account_followup.followup':
            return self.env.context.get('active_id', False)
        company_id = self.env.user.company_id.id
        followp_id = self.env['devit_account_followup.followup'].search(
    ...
    followup_id = fields.Many2one('devit_account_followup.followup', 'Follow-Up',
                                  required=True, readonly=True,
                                  default=_get_followup)
    partner_ids = fields.Many2many('devit_account_followup.stat.by.partner',
    ...
    def process_partners(self, partner_ids, data):
        ...
        nbprints = 0
        resulttext = " "
        for partner in self.env['devit_account_followup.stat.by.partner'].browse(
                partner_ids):
    ...
    def clear_manual_actions(self, partner_list):
        ...
        partner_list_ids = [partner.partner_id.id for partner in self.env[
            'devit_account_followup.stat.by.partner'].browse(partner_list)]
    ...
    def do_process(self):
        ...
        resource_id = self.env.ref(
            'devit_account_followup.view_devit_account_followup_sending_results')
        ...
        return {
            ...
            'context': context,
            'view_mode': 'tree,form',
            'res_model': 'devit_account_followup.sending.results',
    ...
    def _get_partners_followp(self):
        ...
        self._cr.execute(
            '''SELECT *
            FROM devit_account_followup_followup_line
            WHERE followup_id=%s
